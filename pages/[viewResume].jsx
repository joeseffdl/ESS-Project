...
function View() {
  ...
  const displayTemplate = () => {
    ...
    if (resumeDocument.type == "Resume")
      return (
        <OutputResume
          resumeDataProps={resumeDocument.resumeData}
          resumeProperties={resumeFormat}
        />
      );
  };
  ...
  return (
    <div className="w-full flex flex-col xl:flex-row">
      <div
        className={`bg-primary border-t-2 border-secondary-focus w-full p-5 ${
          routeDataID ? "flex" : "hidden"
        } lg:flex justify-center`}
      >
        <div
          className="w-[8.5in] min-h-[11in]  bg-white  text-black"
          ref={componentRef}
        >
          {displayTemplate()}
        </div>
      ...
      <button
        disabled={_.isEmpty(resumeDocument)}
        className="p-2.5 bg-secondary border-t-2 border-x-2 border-secondary-focus text-2xl font-bold"
        onClick={handlePrint}
      >
        <GrDocumentPdf />
      </button>
