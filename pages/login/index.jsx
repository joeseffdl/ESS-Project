    ...
    <main className="w-screen h-screen bg-slate-900">
      <nav className="py-8 font-bold">
        <div
          ...
          className="text-3xl text-white absolute right-10 top-4 cursor-pointer md:hidden"
        >
          <span>{open ? <AiOutlineClose /> : <AiOutlineMenu />}</span>
        </div>
        <ul
          className={`px-10 md:flex md:items-center md:justify-between md:px-24 ${
            open ? "absolute mt-10 md:mt-0" : "hidden"
          }`}
        >
          <li className="hover:translate-x-2 md:hover:-translate-x-2 ease-in-out duration-300">
            <Link href="/">
              <p className="md:text-4xl text-3xl text-white font-extrabold">
                Oregen
              </p>
            </Link>
          </li>
        ...
        <div className="container w-full">
          <div className="h-full flex flex-col gap-3 justify-center items-center p-10">
            <div>
              ...
              <h2
                className="text-lg text-center text-gray-200 mb-3
                                md:text-xl lg:text-2xl"
              >
                Log in to Oregen to continue
              </h2>
            </div>
