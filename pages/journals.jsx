...
import JournalsCollection from "../components/JournalsCollection"
...
function Journals() {
    ...
    const getJournals = async () => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            setAllJournals(snapshot.docs.map((doc) => ({
                ...doc.data(),
                id: doc.id
            })))
        })
                ...
                <div className="bg-white p-5 rounded-2xl divide-y-2">
                    ...
                    <div>
                        {allJournals.map((journal) => (
                            <>
                                <JournalsCollection {...journal} key={journal.id}>
                                    {user.uid == journal.user
                                        ? (
...
Journals.getLayout = function PageLayout(page) {
    return (
        <>
            <AppNavigation />
            {page}
