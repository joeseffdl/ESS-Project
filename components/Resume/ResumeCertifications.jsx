...
function ResumeCertifications() {
  ...
  const updateResumeCertifications = resumeDataStore(
    (state) => state.updateResumeCertifications
  );
  ...
  // States Functions
  const addCertifications = resumeCertificationStore(
    (state) => state.addCertifications
  );
  ...
  const toNextSection = async (e) => {
    ...
    if (_.isUndefined(router.query.id) && _.isEmpty(userCertifications)) {
      ...
      !_.isEqual(initialResumeData.certifications, resumeData.certifications)
    ) {
      try {
