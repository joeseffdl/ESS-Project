...
function ResumeProfileSummary() {
  ...
  const updateResumeProfileSummary = resumeDataStore(
    (state) => state.updateResumeProfileSummary
  );
  ...
  // States Functions
  const setProfileSummary = resumeProfileSummaryStore(
  ...
  const toSubmit = async (e) => {
    ...
    if (_.isUndefined(router.query.id) && _.isEmpty(profileSummary)) {
      ...
      router.query.id &&
      !_.isEqual(initialResumeData.profileSummary, resumeData.profileSummary)
    ) {
      try {
    ...
    <FormWindow onSubmit={toSubmit} formTitle="Resume Profile Summary">
      ...
      <div className="w-full flex flex-col sm:justify-between gap-5">
        ...
        <button
          ...
          className="btn btn-sm sm:btn-md btn-outline btn-accent"
          disabled={_.isEqual(
            initialResumeData.profileSummary,
