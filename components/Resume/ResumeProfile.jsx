...
function ResumeProfile() {
  ...
  const updateResumePersonalInformation = resumeDataStore(
    (state) => state.updateResumePersonalInformation
  );
  ...
  // States
  const personalInformation = resumePersonalInformationStore(
    (state) => state.personalInformation
  );

  // State Functions
  const updatePersonalInformation = resumePersonalInformationStore(
  ...
  const toResumeExperience = async (e) => {
    ...
    if (
      ...
      _.isEmpty(personalInformation.emailAddress)
    ) {
      ...
      router.query.id &&
      !_.isEqual(
        initialResumeData.personalInformation,
  ...
  return (
    <FormWindow
      onSubmit={toResumeExperience}
      formTitle={router.query.id ? "Editing Resume Header" : "Profile Header"}
    >
      <div
      ...
      >
        <input
          ...
          value={
            router.query.id
              ? resumeData.personalInformation.firstname ?? "Fetching data"
        ...
        <input
          ...
          value={
            router.query.id
              ? resumeData.personalInformation.surname ?? "Fetching data"
      ...
      >
        <input
          ...
          value={
            router.query.id
              ? resumeData.personalInformation.city ?? "Fetching data"
        ...
        <input
          ...
          value={
            router.query.id
              ? resumeData.personalInformation.country ?? "Fetching data"
        ...
        <input
          ...
          value={
            router.query.id
              ? resumeData.personalInformation.postalCode ?? "Fetching data"
      ...
      >
        ...
        <input
          ...
          value={
            router.query.id
              ? resumeData.personalInformation.emailAddress ?? "Fetching data"
      ...
      <div className="w-full flex flex-col sm:flex-row justify-between gap-5">
        ...
        <button
          type="submit"
          disabled={
            router.query.id && _.isEmpty(resumeData.personalInformation)
          }
          className="btn btn-sm sm:btn-md btn-outline btn-accent rounded-lg"
        >
          {router.query.id &&
