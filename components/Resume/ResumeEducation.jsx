...
import {
  ...
  fillingForm,
  modalSection,
  resumeDataEducationalBackgroundIndexStore,
  resumeDataStore,
...
function ResumeEducation() {
  ...
  const indexValue = resumeDataEducationalBackgroundIndexStore(
    (state) => state.indexValue
  );

  // Edit States Functions
  const updateResumeEducation = resumeDataStore(
    (state) => state.updateResumeEducation
  );
  const updateResumeEducationDetailsArray = resumeDataStore(
    (state) => state.updateResumeEducationDetailsArray
  );
  const deleteResumeEducation = resumeDataStore(
    (state) => state.deleteResumeEducation
  );
  const clearResumeEducationField = resumeDataStore(
    (state) => state.clearResumeEducationField
  );
  const incrementIndexValue = resumeDataEducationalBackgroundIndexStore(
    (state) => state.incrementIndexValue
  );
  const decrementIndexValue = resumeDataEducationalBackgroundIndexStore(
  ...
  const addEducationDetailsArray = resumeEducationStore(
    (state) => state.addEducationDetailsArray
  );
  const updateEducationField = resumeEducationStore(
    (state) => state.updateEducationField
  );
  const addEducationalBackgroundArray = resumeEducationStore(
    (state) => state.addEducationalBackgroundArray
  );
  const clearEducationField = resumeEducationStore(
    (state) => state.clearEducationField
  );
  ...
  const fillingFormState = async (e) => {
    ...
    if (
      ...
      _.isEmpty(educationField.institutionName && educationField.degreeType)
    ) {
      ...
      router.query.id &&
      _.isEmpty(
        resumeData.educationalBackground[indexValue]?.institutionName &&
          resumeData.educationalBackground[indexValue]?.degreeType
      )
    ) {
      toast.error("Please enter all required fields 😞");
      return;
      ...
      router.query.id &&
      !_.isEqual(
        initialResumeData.educationalBackground,
        resumeData.educationalBackground
      )
    ) {
      try {
  ...
  if (fillingFormValue && !modalSectionValue) {
        ...
        >
          <input
            className={`input rounded-lg focus:outline-none w-full border-2 ${
              router.query.id
                ? resumeData.educationalBackground[indexValue]
                    ?.institutionName == ""
                  ? "input-warning"
                  : "input-success"
          ...
          <input
            ...
            value={
              router.query.id
                ? resumeData.educationalBackground[indexValue]
                    ?.institutionLocation ?? ""
                : educationField.institutionLocation
            }
              ...
              router.query.id
                ? resumeData.educationalBackground[indexValue]?.degreeType ===
                  ...
                  resumeData.educationalBackground[indexValue]?.degreeType ===
                    undefined
                : educationField.degreeType === "" ||
                  educationField.degreeType === "High School Diploma" ||
                  educationField.degreeType === "GED" ||
                  educationField.degreeType === "No Degree"
            }
            className="input rounded-lg focus:outline-none w-full border-2 "
            placeholder="Field of Study"
            value={
              router.query.id
                ? resumeData.educationalBackground[indexValue]?.fieldOfStudy ??
                  ""
            ...
            <option
              ...
              selected={
                router.query.id
                  ? resumeData.educationalBackground[indexValue]
                      ?.graduationMonth ?? "" == ""
                  : educationField.graduationMonth == ""
              }
            >
              Graduation Month
            </option>
          ...
          {router.query.id ? (
            <div className="w-full flex justify-end gap-5 mt-3 px-2">
              <button
                ...
                disabled={resumeData.educationalBackground.length < 1}
                onClick={deleteIndex}
              >
                ☠️
              </button>
              ...
              <button
                className=" btn btn-xs btn-outline"
                disabled={
                  indexValue >= resumeData.educationalBackground.length - 1
                }
    ...
    return (
      <FormWindow
        onSubmit={toAddEducationSection}
        formTitle="Education Description"
      >
        <textarea
        ...
        >
          {router.query.id
            ? resumeData.educationalBackground[indexValue].description?.join(
                "\n"
              )
            : description.value}
        </textarea>
    ...
    return (
      <FormWindow
        onSubmit={toResumeSkills}
        formTitle="Preview Educational Attainment"
      >
        <div className="w-full flex flex-col sm:justify-between gap-5">
          <button
            className="btn btn-sm sm:btn-md btn-outline btn-accent rounded-lg"
