...
function ResumeSkills() {
  ...
  const updateResumeSkills = resumeDataStore(
    (state) => state.updateResumeSkills
  );
  ...
  return (
    <FormWindow
      onSubmit={toProfileSummarySection}
      formTitle={
        router.query.id
          ? "Editing Resume Skills and Expertise"
    ...
    >
      ...
      <div className="w-full flex flex-col sm:justify-between gap-5">
        ...
        >
          {router.query.id &&
          !_.isEqual(initialResumeData.skills, resumeData.skills)
            ? "Update"
            : "Continue"}
        </button>
      </div>
