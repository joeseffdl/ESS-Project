...
function ResumePortfolios() {
  ...
  const updateResumePortfolio = resumeDataStore(
    (state) => state.updateResumePortfolio
  );
  ...
  const profileSummary = resumeProfileSummaryStore(
    (state) => state.profileSummary
  );
  const certifications = resumeCertificationStore(
  ...
  const toNextSection = async (e) => {
    ...
    if (_.isUndefined(router.query.id) && _.isEmpty(portfolio)) {
      ...
      router.query.id &&
      !_.isEqual(initialResumeData.portfolio, resumeData.portfolio)
    ) {
      try {
