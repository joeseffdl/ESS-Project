      ...
      >
        <div className="font-bold h-10 text-2xl cursor-pointer flex items-end">
          <div className="flex items-end">
            <Link
              href="/"
              className="text-4xl font-extrabold hover:text-secondary-focus"
            >
              Oregen
            </Link>{" "}
        ...
        <div
          ...
          className="text-3xl text-neutral-focus cursor-pointer md:hidden"
        >
          <span>{open ? <AiOutlineClose /> : <AiOutlineMenu />}</span>
        </div>
        <ul
          className={`h-10 pb-5 absolute left-0 w-full
        ...
        >
          {Links.map((link) => (
            <li
              key={link.name}
              className="text-2xl mt-5 pt-3 px-10
                                md:ml-8 md:mt-0 md:border-none md:px-0 md:pt-0 "
            >
              <Link
                onClick={() => setOpen(!open)}
                href={link.link}
                className="md:hover:text-secondary-focus hover:text-accent-focus duration-150"
              >
                {link.name}
              </Link>
            </li>
