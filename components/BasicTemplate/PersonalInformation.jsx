      ...
      {viewingResume ? (
        ...
        <section className="w-full ">
          <div className="text-center text-3xl font-semibold my-4 tracking-widest">
            {router.query.id
              ? `${
                  resumeData.personalInformation.firstname?.toUpperCase() ??
                  "Fetching"
                } ${
                  resumeData.personalInformation.surname?.toUpperCase() ??
                  "data"
                }`
              : personalInformation.firstname || personalInformation.surname
              ? `${personalInformation.firstname.toUpperCase()} ${personalInformation.surname.toUpperCase()}`
              : "FIRST NAME SURNAME"}
          </div>
          <div className="grid grid-flow-col auto-cols-max justify-center text-center mb-6 mx-32 divide-x">
            {router.query.id ? (
              <>
                ...
                {(resumeData.personalInformation.city ||
                  resumeData.personalInformation.country) && (
                  <div className="px-2">
                    {resumeData.personalInformation.city}
                    {resumeData.personalInformation.city && ","}{" "}
                    {resumeData.personalInformation.country}{" "}
                    {resumeData.personalInformation.postalCode}
                  </div>
                )}
              </>
            ) : (
              <>
                {personalInformation.emailAddress && (
                  <div className="px-2">{personalInformation.emailAddress}</div>
                )}
                {personalInformation.phoneNumber && (
                  <div className="px-2">{personalInformation.phoneNumber}</div>
                )}
